...
#define OPEN_SCALE_TRIAL \
  5  // how many times we try to open the scale before exit
#define CLOST_SCALE_TRIAL \
  6  // how many times we try to close the scale before exit

#define SCALE_SIGNATURE \
  255  // signature value of the first byte given by manufacturer
...
int openScale(FILE *log)
{
  ...
  scale_settings.c_cflag |=
      CLOCAL | CREAD;                // ignore modem line and enable receiver
  ...
  if (!cfsetispeed(&scale_settings, B9600) &&
    ...
    {
      errorLogging(
          "error while setting scale with error code \n", log, "OPEN_SCALE");
  ...
  if (select(scale + 1, inputSet, NULL, NULL, timeOut))
    {
      ...
      if (temp != 6 && temp >= 0)
      ...
      else if (temp < 0)
        {
          ...
          errorLogging(strerror(errno), log, "READ_RAW");
          ...
          else
            {
              if (prevBuffer[0] == buffer[2] && prevBuffer[1] == buffer[3] &&
              ...
              else
                {
                  ...
                  if (isOverflow)
                    {
                      errorLogging("Scale overflow", log, "READING_PARSING");
                  ...
                  result = digit1 + digit2 * 10 + digit3 * 100 + digit4 * 1000 +
                           digit5 * 10000 + digit6 * 100000;
                  result =
                      (result / (pow(10, decimal_point - 1))) * UNIT_CONVERSION;
...
int main(void)
{
  ...
  errorLogging("Testing", log, "OPEN_FILE");
  assert(log);
